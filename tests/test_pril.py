...
def test_create_client(client):
    data = {"name": "Alice", "surname": "Smith", "credit_card": "1111-2222-3333-4444", "car_number": "XYZ789"}
...
def test_create_parking(client):
    data = {"address": "New Parking", "opened": "true", "count_places": 20, "count_available_places": 20}
...
def test_enter_parking(client, db_session):
    ...
    response = client.post(f"/client_parkings?client_id={client_id}&parking_id={parking_id}")
...
def test_exit_parking(client, db_session):
    client_id = 1
    parking_id = 1
    response = client.delete("/client_parkings", data={"client_id": client_id, "parking_id": parking_id})
    assert response.status_code == 200
    parking = db_session.get(Parking, parking_id)
